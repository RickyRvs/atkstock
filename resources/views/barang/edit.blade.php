...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div class="flex items-center gap-3">
        ...
        <div class="min-w-0">
            <h2 class="text-lg font-semibold text-gray-800 truncate">Edit Barang</h2>
            <p class="text-sm text-gray-500 truncate">{{ $barang->nama_barang }}</p>
        </div>
    </div>
            ...
            <form method="POST" action="{{ route('barang.update', $barang) }}" class="space-y-5">
                ...
                <div>
                    ...
                    <textarea name="keterangan" rows="3"
                              class="w-full border border-gray-200 rounded-lg px-3.5 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-200 focus:border-blue-400 transition-colors resize-none">{{ old('keterangan', $barang->keterangan) }}</textarea>
                </div>

                <label for="is_active" class="flex items-center gap-3 p-4 bg-gray-50 rounded-xl cursor-pointer hover:bg-gray-100 transition-colors">
                    <input type="hidden" name="is_active" value="0">
                    <input type="checkbox" name="is_active" id="is_active" value="1"
                           {{ old('is_active', $barang->is_active) ? 'checked' : '' }}
                           class="w-4 h-4 text-blue-600 rounded focus:ring-blue-300">
                    <div>
                        <p class="text-sm font-medium text-gray-700">Barang masih aktif</p>
                        <p class="text-xs text-gray-400">Barang nonaktif tidak muncul di transaksi baru</p>
                    </div>
                </label>
